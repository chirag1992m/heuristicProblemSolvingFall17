...
class Client:
  ...
  def cluster(self):
    k = self.num_stone
    centroids = [[random.randint(0,999), random.randint(0, 999)] for i in range(k)]
    ...
    while(last != centroids):
      ...
      new_clusters = [[0,0] for i in range(k)]
      for l in range(k):
  ...
  def start(self):
    ...
    while True:
      ...
      # make move
      print(self.__getMove(cur))
      my_move_row, my_move_col = self.__getMove(cur)
      cur += 1
